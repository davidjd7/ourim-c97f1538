...
export function KanbanBoard({
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 min-h-[600px]">
        {columns.map(column => <Card key={column.id} className={`${column.color} h-fit`}>
            ...
            <CardContent className="space-y-3">
              {column.investments.length === 0 ? <div className="text-center py-8 text-muted-foreground text-sm">Drop
1</div> : column.investments.map(investment => <InvestmentCard key={investment.id} investment={investment} />)}
            </CardContent>
          </Card>)}
      </div>
    </div>;
