...
interface MinimalStatusProgressProps {
...
const statusConfig = {
  RECU: { label: 'Reçu', step: 1, className: 'status-recu', color: 'bg-orange-500' },
  DUE_DIL: { label: 'Due Dil', step: 2, className: 'status-due-dil', color: 'bg-blue-500' },
  INVESTI: { label: 'Investi', step: 3, className: 'status-investi', color: 'bg-green-500' },
  VENDU: { label: 'Vendu', step: 4, className: 'status-vendu', color: 'bg-purple-500' },
  DROP: { label: 'Drop', step: 0, className: 'status-drop', color: 'bg-red-500' }
};

const statusOrder: InvestmentStatus[] = ['RECU', 'DUE_DIL', 'INVESTI', 'VENDU'];

export function MinimalStatusProgress({ 
  currentStatus, 
  onStatusChange, 
  netVendeur = 0, 
  agent = 0, 
  honoNotaire = 0 
}: MinimalStatusProgressProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      {/* Progression minimaliste horizontale */}
...
export { MinimalStatusProgress as StatusProgressActions };

// Actions séparées pour l'en-tête
